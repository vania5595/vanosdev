...
int VideoRamVector(int x,int y){
	return y*1024+x;
}

void render8Bitmap(int vrv,char bm){
	#define rop(_n) vram[vrv+ _n ]=((bm >> 7- _n ) & 1) * 0xFFFFFFFF;
	rop(0)
	rop(1)
	rop(2)
	rop(3)
...
void renderChar(int vrv,char c){
	for(int i=0;i<8;i++){
		render8Bitmap(vrv+1024*i,font[c][i]);
	}
}

void InitVESA(struct kernel_structure *ks){
	vram=ks->multiboot->vbe_mode_info->physbase;
} 
